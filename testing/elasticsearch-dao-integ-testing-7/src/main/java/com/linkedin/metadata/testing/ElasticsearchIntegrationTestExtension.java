...
final class ElasticsearchIntegrationTestExtension
  ...
  private void cleanUp(@Nonnull List<SearchIndex<?>> indices) {
    for (SearchIndex<?> i : indices) {
      final DeleteIndexRequest deleteIndexRequest = new DeleteIndexRequest(i.getName());
      try {
        _connection.getRestHighLevelClient().indices().delete(deleteIndexRequest, RequestOptions.DEFAULT);
      } catch (IOException e) {
