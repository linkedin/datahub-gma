...
public final class SearchIndex<DOCUMENT extends RecordTemplate> {
  ...
  public SearchIndex(@Nonnull Class<DOCUMENT> documentClass, @Nonnull ElasticsearchConnection connection,
    ...
    final BulkProcessor bulkProcessor = BulkProcessor.builder(
        (request, listener) -> connection.getRestHighLevelClient().bulkAsync(request, RequestOptions.DEFAULT, listener),
        bulkListener).build();
  ...
  public void setSettings(@Nonnull String settingsJson) {
    final UpdateSettingsRequest updateSettingsRequest =
        new UpdateSettingsRequest(_name).settings(settingsJson, XContentType.JSON);
    try {
      _connection.getRestHighLevelClient().indices().putSettings(updateSettingsRequest, RequestOptions.DEFAULT);
    } catch (IOException e) {
  ...
  public void setMappings(@Nonnull String mappingsJson) {
    final PutMappingRequest putMappingRequest = new PutMappingRequest(_name).source(mappingsJson, XContentType.JSON);
    try {
      _connection.getRestHighLevelClient().indices().putMapping(putMappingRequest, RequestOptions.DEFAULT);
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
  }
