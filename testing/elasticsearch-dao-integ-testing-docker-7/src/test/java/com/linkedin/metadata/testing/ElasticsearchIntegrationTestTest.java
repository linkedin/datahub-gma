...
public class ElasticsearchIntegrationTestTest {
  ...
  @Test
  @Order(2)
  public void instanceIsCleanedUpBetweenMethods() throws Exception {
    // given
    final IndicesClient indicesClient = searchIndex.getConnection().getRestHighLevelClient().indices();

    // when
    final boolean exists = indicesClient.exists(new GetIndexRequest(_methodIndexName), RequestOptions.DEFAULT);
  ...
  public void settingsAndMappingsAnnotation() throws Exception {
    ...
    final GetSettingsResponse response = searchIndex.getConnection()
        ...
        .indices()
        .getSettings(new GetSettingsRequest().indices(searchIndex.getName()), RequestOptions.DEFAULT);
