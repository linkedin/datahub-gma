...
public class EbeanLocalDAO<ASPECT_UNION extends UnionTemplate, URN extends Urn>
   ...
   *
   * <p>TODO: once MULTI_ASPECT is fully rolled out and validated, remove the PER_ASPECT and DUAL
   * strategies (and the {@link AspectReadStrategy} switch) so the multi-aspect path becomes the only
   * read path, for code cleanup.</p>
   */
  private List<EbeanMetadataAspect> newSchemaBatchGet(@Nonnull List<AspectKey<URN, ? extends RecordTemplate>> keys,
    ...
    switch (_aspectReadStrategy) {
      ...
      case DUAL:
        ...
        // The multi-aspect read is a shadow path used only to validate parity against the safe
        // per-aspect result. Any failure in it (e.g. a SQL error in the bundled read) must never break
        // the primary read, so wrap the shadow read + comparison and downgrade failures to a warning.
        try {
          ...
          log.warn("Multi-aspect shadow read failed during DUAL comparison (keysCount={}, position={}, keys={}); "
              + "returning per-aspect result.", keysCount, position, keys.size(), e);
        }
        return perAspect;
      case PER_ASPECT:
      default:
